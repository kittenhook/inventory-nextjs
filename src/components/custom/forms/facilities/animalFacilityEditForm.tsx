...
export default function AnimalFacEditForm(pageProps: pageProps) {
	const { toast } = useToast();
	const router = useRouter();
	const form = useForm<z.infer<typeof newTreeSchema>>({
		resolver: zodResolver(newTreeSchema),
		defaultValues: {
			ten: pageProps.facilities.ten,
			maDinhDanhLoaiDongVat:
				pageProps.facilities.maDinhDanhLoaiDongVat || "",
			maDinhDanhNguoiPhuTrach:
				pageProps.facilities.maDinhDanhNguoiPhuTrach || "",
			maDinhDanhQuan: pageProps.facilities.maDinhDanhQuan || "",
		},
	});

	async function deleteData() {
		const response = await fetch(
			`/api/database/coSo/DongVat/${pageProps.facilities.maDinhDanh}`
		);
	}
	...
	return (
		<div className='space-y-3'>
			<span className='text-2xl font-semibold tracking-tight'>
				Cập nhật cơ sở lưu trữ động vật.
			</span>
			<Form {...form}>
				<form
					id='treeEditForm'
